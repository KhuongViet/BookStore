...
if ($productName == "" || $catId == "" || $brandId == "" || $body == "" || $price == "" || $file_name == "" || $type == "") {
	...
	return $msg;
     ...
     echo "<span class='error'>Kích thước hình ảnh phải nhỏ hơn 1MB!
     </span>";
	 ...
	 $inserted_row = $this->db->insert($query);
			if ($inserted_row) {
				$msg = "<span class='success'>Sản phẩm được chèn thành công.</span>";
				return $msg;
			} else{
				$msg = "<span class='error'>Sản phẩm Chưa được đưa vào.</span>";
				return $msg;
		}
		}
...
if ($file_size >1048567) {
	 ...
	 $updatedted_row = $this->db->update($query);
			if ($updatedted_row) {
				$msg = "<span class='success'>Sản phẩm được cập nhật thành công.</span>";
				return $msg;
			} else{
				$msg = "<span class='error'>Sản phẩm chưa được cập nhật</span>";
				return $msg;
		}
		}
}else{
...
$deldata = $this->db->delete($delquery);
	if ($deldata) {
		$msg = "<span class='success'>Sản phẩm đã bị xóa thành công.</span>";
				return $msg;
	}else{
$msg = "<span class='error'>Sản phẩm chưa bị xóa!</span>";
				return $msg;
...
public function latestFromSeasonalCakes(){
	$query = "SELECT * FROM tbl_product WHERE brandId = '1' ORDER BY productId DESC LIMIT 1";
	$result = $this->db->select($query);
	return $result;
}
public function latestFromSweet(){
	$query = "SELECT * FROM tbl_product WHERE brandId = '2' ORDER BY productId DESC LIMIT 1";
	$result = $this->db->select($query);
	return $result;
}
public function latestFromBread(){
	$query = "SELECT * FROM tbl_product WHERE brandId = '3' ORDER BY productId DESC LIMIT 1";
	$result = $this->db->select($query);
	return $result;
}
public function latestFromCream(){
...
// 	if ($check) {
// 		$msg = "<span class='error'>Đã thêm !</span>";
// 				return $msg;
// 	}
...
// 	$msg = "<span class='success'>Thêm ! Kiểm tra trang so sánh</span>";
// 				return $msg;
// 	}else{
// $msg = "<span class='error'>Không thêm vào !</span>";
// 				return $msg;
...
public function saveWishListData($id,$cmrId){
	...
	$msg = "<span class='success'>Thêm ! Kiểm tra trang danh sách yêu thích</span>";
		return $msg;
	}else{
   $msg = "<span class='error'>Không thêm vào !</span>";
		return $msg;
...
public function getTopbrandSeasonalCakes(){

	$query = "SELECT * FROM tbl_product WHERE brandId = '1' ORDER BY productId ";
	$result = $this->db->select($query);
	return $result;
}
public function getTopbrandsweet(){

	$query = "SELECT * FROM tbl_product WHERE brandId = '2' ORDER BY productId ";
	$result = $this->db->select($query);
	return $result;
}

public function getTopbrandbread(){

	$query = "SELECT * FROM tbl_product WHERE brandId = '3' ORDER BY productId ";
	$result = $this->db->select($query);
	return $result;
}

public function getTopbrandcream(){

	$query = "SELECT * FROM tbl_product WHERE brandId = '4' ORDER BY productId ";
	$result = $this->db->select($query);
	return $result;
}
}
